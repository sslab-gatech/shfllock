...
if step > 1:
	data_points.insert(0, 1)
        data_points.insert(1, 2)
        data_points.insert(2, 4)
        data_points.insert(3, 8)
        data_points.insert(4, 12)

# also add some extra cases
data_points.append(384)
data_points.append(768)
data_points.append(1536)

for i in data_points:
	c = './%s_processes -t %d -s %d -p' % (cmd, i, duration)
	before = linux_stat()
	...
	c = './%s_threads -t %d -s %d -p' % (cmd, i, duration)
	before = linux_stat()
	pipe = subprocess.Popen(setarch + ' ' + c, shell=True, stdout=subprocess.PIPE).stdout
	threads_avg = -1
	for line in pipe.readlines():
