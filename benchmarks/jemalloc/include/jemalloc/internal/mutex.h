...
#define JEMALLOC_TICKET_LOCK 1

#if defined(JEMALLOC_TICKET_LOCK)
#define CACHE_LINE_SIZE (128)
#define ____cacheline_aligned  __attribute__ ((aligned (CACHE_LINE_SIZE)))

typedef struct ticketlock_t {
	union {
		volatile uint64_t tickets;
		struct {
			volatile uint32_t head;
			volatile uint32_t tail;
		};
	};
	/* volatile uint32_t head ____cacheline_aligned; */
	/* volatile uint32_t tail ____cacheline_aligned; */
} ticketlock_t;

#define TICKETLOCK_INITIALIZER {.head = 0, .tail = 0}

static inline int ticketlock_trylock(ticketlock_t *lock)
{
	...
	if (__sync_val_compare_and_swap(la, cmp, cmp_new) == cmp) {
		return 0;
	}
	return 1;
}
...
{
	uint32_t my_ticket = __sync_fetch_and_add(&(lock->tail), 1);
...
#  endif
			...
			os_unfair_lock		lock;
#elif (defined(JEMALLOC_TICKET_LOCK))
			ticketlock_t 		lock;
...
#elif (defined(JEMALLOC_TICKET_LOCK))
#    define MALLOC_MUTEX_LOCK(m)    ticketlock_lock(&(m)->lock)
#    define MALLOC_MUTEX_UNLOCK(m)  ticketlock_unlock(&(m)->lock)
#    define MALLOC_MUTEX_TRYLOCK(m) (ticketlock_trylock(&(m)->lock) != 0)
#else
...
#  if defined(JEMALLOC_DEBUG)
#    define MALLOC_MUTEX_INITIALIZER					\
  {{{LOCK_PROF_DATA_INITIALIZER, TICKETLOCK_INITIALIZER, ATOMIC_INIT(false)}}, \
         WITNESS_INITIALIZER("mutex", WITNESS_RANK_OMIT), 0}
#  else
#    define MALLOC_MUTEX_INITIALIZER                      \
  {{{LOCK_PROF_DATA_INITIALIZER, TICKETLOCK_INITIALIZER, ATOMIC_INIT(false)}},  \
