 ...
 *
 *  - Adaptive spinning for rwmutexevlves by Peter Zijlstra. (Ported to mainline
 *    from the -rt tree, where it was originally implemented for rtrwmutexevlves
 *    by Steven Rostedt, based on work by Gregory Haskins, Peter Morreale
 *    and Sven Dietrich.
 *
 * Also see Documentation/locking/rwmutexevlv-design.txt.
 */
...
static int __aqm_lock_slowpath(struct rwmutexevlv *lock, long state);

void
__rwmutexevlv_init(struct rwmutexevlv *lock, const char *name)
{
 ...
 */
static inline bool __rwmutexevlv_trylock(struct rwmutexevlv *lock)
{
	return (atomic_cmpxchg(&lock->val, 0, 1) == 0);
}

static inline void rwmutexevlv_lock(struct rwmutexevlv *lock)
{
...
static inline void rwmutexevlv_unlock(struct rwmutexevlv *lock)
{
	xchg(&lock->locked, 0);
}
...
static inline void enable_stealing(struct rwmutexevlv *lock)
{
        atomic_andnot(_AQ_MCS_NOSTEAL_VAL, &lock->val);
}

static inline void unlock_and_enable_stealing(struct rwmutexevlv *lock)
{
        WRITE_ONCE(lock->locked_no_stealing, 0);
}

static inline void disable_stealing(struct rwmutexevlv *lock)
{
        atomic_fetch_or_acquire(_AQ_MCS_NOSTEAL_VAL, &lock->val);
}

static inline u8 is_stealing_disabled(struct rwmutexevlv *lock)
{
        return smp_load_acquire(&lock->no_stealing);
}

static inline void set_sleader(struct rwaqmevlv_node *node, struct rwaqmevlv_node *qend)
{
...
static inline void clear_sleader(struct rwaqmevlv_node *node)
{
        node->sleader = 0;
}

static inline void set_waitcount(struct rwaqmevlv_node *node, int count)
{
        smp_store_release(&node->wcount, count);
}

static inline void wake_up_waiter(struct rwaqmevlv_node *node)
{
...
static int force_update_node(struct rwaqmevlv_node *node, u8 state)
{
#if 1
...
static inline void park_waiter(struct rwaqmevlv_node *node, long state)
{
...
static inline void shuffle_waiters(struct rwmutexevlv *lock, struct rwaqmevlv_node *node,
				   int is_next_waiter)
{
        struct rwaqmevlv_node *curr, *prev, *next, *last, *sleader, *qend;
        int nid;
...
static int __aqm_lock_slowpath(struct rwmutexevlv *lock, long state)
{
	struct rwaqmevlv_node snode ____cacheline_aligned;
	struct rwaqmevlv_node *node = &snode;
        struct rwaqmevlv_node *prev, *next;
        u8 plstatus;
	u16 wcount = 0;
	u8 sleader = true;
...
static inline void aqm_read_lock_slowpath(struct rwaqmevlv *sem)
{
	atomic_long_sub(RWAQM_R_BIAS, &sem->cnts);

	rwmutexevlv_lock(&sem->wait_lock);
	atomic_long_add(RWAQM_R_BIAS, &sem->cnts);

	atomic_long_cond_read_acquire(&sem->cnts, !(VAL & RWAQM_W_LOCKED));

	rwmutexevlv_unlock(&sem->wait_lock);
}

void rwaqmevlv_down_read(struct rwaqmevlv *sem)
{
	u64 cnts;
...
void __must_check rwaqmevlv_down_read_killable(struct rwaqmevlv *sem)
{
	/* XXX: Will handle the EINTR later */
	rwaqmevlv_down_read(sem);
}

/*
 * trylock for reading -- returns 1 if successful, 0 if contention
 */
int rwaqmevlv_down_read_trylock(struct rwaqmevlv *sem)
{
	u64 cnts;
...
static inline void aqm_write_lock_slowpath(struct rwaqmevlv *sem)
{
	rwmutexevlv_lock(&sem->wait_lock);
 ...
 * lock for writing
 */
void rwaqmevlv_down_write(struct rwaqmevlv *sem)
{
...
int __must_check rwaqmevlv_down_write_killable(struct rwaqmevlv *sem)
{
	rwaqmevlv_down_write(sem);
	return 0;
}
 ...
 */
int rwaqmevlv_down_write_trylock(struct rwaqmevlv *sem)
{
	u64 cnts;
 ...
 * release a read lock
 */
void rwaqmevlv_up_read(struct rwaqmevlv *sem)
{
 ...
 */
void rwaqmevlv_up_write(struct rwaqmevlv *sem)
{
 ...
 */
void rwaqmevlv_downgrade_write(struct rwaqmevlv *sem)
{
	/*
	 * Two ways to do it:
